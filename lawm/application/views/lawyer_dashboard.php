                                    ...
                                    <table class="table table-hover table-bordered table-striped <?php if($this->session->userdata('language_id')!=1){ echo "rtl"; } ?>">
                                        ...
                                        <?php for($s=0;$s<count($lawyer_consultation);$s++){ ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn-u btn btn-u-xs" data-toggle="modal" data-target="#view_module" onclick="getReject('<?=$lawyer_consultation[$s]['id_consultation']?>','consultation')"> <?=$this->lang->line('decline')?></button>
                                                    <button onclick="updateConsultation('<?=$lawyer_consultation[$s]['id_consultation']?>','accepted')" class="btn-u btn btn-u-xs"> <?=$this->lang->line('accept')?></button></td>
                                                <?php } else{ ?>
                                                ...
                                                <td class="head1 attachement" style="text-align:center;">
                                                    ...
                                                    <?php
                                                    $attachment = $this->Mwelcome->getAttachment(array('type' => 'consultation','reference_id' => $lawyer_consultation[$s]['id_consultation']));
                                                    if(!empty($attachment)){
                                                ...
                                                <td>
                                                    <?php /*if($lawyer_consultation[$s]['status']=='accepted'){ */?><!--
                                                        <a href="<?/*=BASE_URL*/?>index.php/Welcome/comments/consultation/<?/*=$this->Mwelcome->encode($lawyer_consultation[$s]['id_consultation'])*/?>" ><i class="expand-list rounded-x fa fa-reply"></i></a>

                                                    <?php /*} */?>
                                                    <a data-target="#view_module" onclick="getModule('consultation','<?/*=$this->Mwelcome->encode($lawyer_consultation[$s]['id_consultation'])*/?>');" data-toggle="modal" href="javascript:;"  ><i class="expand-list rounded-x fa fa-eye"></i></a>-->
                                                    <a  class="btn-u btn btn-u-xs" style="color: white" href="<?=BASE_URL?>index.php/Welcome/comments/consultation/<?=$this->Mwelcome->encode($lawyer_consultation[$s]['id_consultation'])?>" ><?=$this->lang->line('view')?></a>
                    ...
                    <div class="col-sm-12 sm-margin-bottom-30">
                        ...
                        <div class="panel panel-profile">
                            ...
                            <div class="panel-heading overflow-h">
                                ...
                                <h2 class="panel-title heading-sm pull-left"><i class="fa fa-server"></i> <?=$this->lang->line('consultation_all')?></h2>
                                        ...
                                        <table class="table table-hover table-bordered table-striped <?php if($this->session->userdata('language_id')!=1){ echo "rtl"; } ?>">
                                            ...
                                            <?php for($s=0;$s<count($other_consultation);$s++){ ?>
                                                <tr>
                                                    ...
                                                    <td><a style="color:blue;" href="javascript:;" data-target="#view_module" data-toggle="modal" onclick="getModule('consultation','<?=$this->Mwelcome->encode($other_consultation[$s]['id_consultation'])?>');"><?=$other_consultation[$s]['user_name']?></a></td>
                                                    <td><a style="color:blue;" href="javascript:;" data-target="#view_module" data-toggle="modal" onclick="getModule('consultation','<?=$this->Mwelcome->encode($other_consultation[$s]['id_consultation'])?>');"><?=$other_consultation[$s]['subject']?></a></td>
                                                    <td class="td-width"><?=$other_consultation[$s]['created_date_time']?></td>
                                                    <td><button onclick="getPick('<?=$other_consultation[$s]['id_consultation']?>','consultation')" class="btn-u btn-u-blue btn-u-xs"> <?=$this->lang->line('pick')?></button>  </td>
                                ...
                                <div class="table-responsive">
                                    ...
                                    <div id="mail">
                                        ...
                                        <table class="table table-hover table-bordered table-striped open <?php if($this->session->userdata('language_id')!=1){ echo "rtl"; } ?>" style="border-collapse:collapse; margin-top:0px; width:100%;" border="0" cellpadding="5" cellspacing="3">
                                            ...
                                            <tbody>
                                            <?php $closed_cases = $this->Mwelcome->consultation(array('lawyer_id' => $this->session->userdata('user_id'),'status' => 'closed'));
                                            if(!empty($closed_cases)){
                                                for($s=0;$s<count($closed_cases);$s++){
                                                    $attachment = $this->Mwelcome->getAttachment(array('type' => 'consultation','reference_id' => $closed_cases[$s]['id_consultation']));
                                                    ?>
                                                    <tr>
                                                        ...
                                                        <td class="head0" style="text-align:left;">
                                                            <?php $rating = $this->Mwelcome->getRating(array('reference_type' => 'consultation','reference_id' => $closed_cases[$s]['id_consultation']));
                                                                if(!empty($rating)){
                                                            ?>
