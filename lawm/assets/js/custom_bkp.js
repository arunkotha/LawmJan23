...
function updateProfile()
{
    $.ajax({
        ...
        data: {first_name:$('#first_name').val(),last_name:$('#last_name').val(),father_name:$('#father_name').val(),gender:$('#gender').val(),country_id:$('#country').val(),speciality_id:$('#speciality').val()},
        dataType: 'json',
        success:function(res){
...
function validNewConsultation()
{
    ...
    if(img==1){
        alert('Invalid file format..only image,pdf,doc files are allowed');
        return false;
    }
...
function getCloseConPopup(txt,type,id)
{
    ...
    $('#close-consult #ref_type').val(type);
    $('#close-consult #ref_id').val(id);
}
...
function addPartner()
{
    var html = '<div><section style="padding-left: 0px;" class="col col-6"><label for="date" class="input"><input type="text" placeholder="Partner name" id="partner_name[]" name="partner_name[]" value=""></label></section>';
    html+='<section class="col col-5"><label for="date" class="input"><input type="text" placeholder="Partner id" id="partner_id[]" name="partner_id[]" value="" class="form-control"></label></section>';
...
function getModule(type,id)
{
    ...
    var html1 = '<table class="view-module table table-bordered"><tr><th>Partner Name</th><th>Partner Id</th></tr>';
    $.ajax({
        async: true,
        type: 'POST',
