    ...
    */
    private function get_ordering()
    {
      ...
      if ($this->ci->input->post('order'))
      {
          ...
          if(isset($_POST['my_order'])){
              $this ->ci->db->order_by($_POST['my_order'],'desc');
          }
    ...
    */
    private function jsonify($result = FALSE)
    {
     ...
     public function query($sql)
     {
         ...
         {
             $result[$s] = array_values((array)$result[$s]);
         }
         $sOutput = array
         (
             'draw'                => intval($this->ci->input->post('draw')),
             'recordsTotal'        => $iTotalRecords[0]->TOTAL,
             'recordsFiltered'     => $iTotalDisplayRecords[0]->TOTAL,
             'data'                => $result
         );
