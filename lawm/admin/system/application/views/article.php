...
<div class="row">
    ...
    <div style="float: right; height: 35px; margin: 0px 30px 11px 0px; width: 150px;">
        <button onclick="getAddArticle();" class="btn btn-primary" data-toggle="modal" data-target="#myModal" ><?=$this->lang->line('add_article')?></button>
    </div>
...
<div class="box-header">
    ...
    <h3 class="box-title"><?=$this->lang->line('article')?>:</h3>
</div><!-- /.box-header -->
<div class="box-body">
...
<tr>
    ...
    <th><?=$this->lang->line('article')?></th>
    <th><?=$this->lang->line('order')?></th>
    <th><?=$this->lang->line('action')?></th>
</tr>
</thead>
<tbody>
<?php for($s=0;$s<count($article_data);$s++){ ?>
    <tr>
        <td><?=$article_data[$s]['language']?></td>
        <td><?=$article_data[$s]['menu_title']?></td>
        <td><?=$article_data[$s]['article_title']?></td>
        <td><?=$article_data[$s]['order']?></td>
        <td>
            <div class="mod-more tooltipsample actions">
                <a id="edit" onclick="editArticle(<?=$article_data[$s]['id_article']?>)" title="edit" data-toggle="modal" data-target="#myModal" class="edit" href="javascript:;"><span class="circle"><i class="fa fa-pencil"></i></span></a></a>
            </div>
        </td>
    </tr>
<?php } ?>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form method="post" action="<?=ADMIN_BASE_URL?>index.php?welcome/articleOperations" enctype="multipart/form-data" onsubmit="return validArticle(this);" >
                ...
                <div class="modal-body" id="model-body">
                    ...
                    <div class="form-group" id="">
                        ...
                        <select class="form-control" name="menu_id" id="menu_id" required>
                            ...
                            <?php /*for($s=0;$s<count($category_drp);$s++){ */?><!--
                                <option value="<?/*=$category_drp[$s]['category_flag']*/?>"><?/*=$category_drp[$s]['category_title']*/?></option>
                            --><?php /*} */?>
                            <?php for($s=0;$s<count($menu_drp);$s++){ ?>
                                <option value="<?=$menu_drp[$s]['id_menu']?>"><?=$menu_drp[$s]['menu_title']?></option>
                            <?php } ?>
                        </select>
